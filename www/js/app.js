...
(function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          for (var i = 0; i < data.length; i++) {
            $optionsSelect.append('<option value="' + data[i]['code'] + '">'
                + data[i]['name'] + '</option>');
          }
        },
        statusCode: {
  ...
  var MobilityChoicesView = (function () {
    ...
    function cancel(e) {
      e.preventDefault();
      if (app.isProfessor()) {
        RejectMobilityChoiceView.render(
            $(this).parent().parent().attr('data-id'));
      } else {
        CancelMobilityChoiceView.render(
            $(this).parent().parent().attr('data-id'));
      }
    }
          ...
          aoColumns: [
            {
              mData: function (o, type) {
                return o.preferenceOrder
                    + '<span class="mc-id" style="display: none;">' + o.id
                    + '</span>';
              },
              title: '# Préférence'
            },
            ...
            {
              mData: function (o, type) {
                return ((o.term === 1) ? 'Septembre' : 'Février') + ' '
                    + o.academicYear
              },
              title: 'Départ'
            },
    ...
    function render() {
      ...
      bindAll();
      $el.find('h2').html(
          ((app.isProfessor()) ? 'Gestions des demandes de mobilité'
              : 'Mes demandes de mobilité'));
      updateWindowTitle();
      $el.show();
    }
  ...
  var CreateMobilityChoiceView = (function () {
        ...
        success: function (data) {
          for (var i = 0; i < data['data'].length; i++) {
            if (data['data'][i]['role'] === 'Student') {
              $studentsSelect.append('<option value="' + data['data'][i]['id']
                  + '">' + data['data'][i]['lastName'] + ' '
                  + data['data'][i]['firstName'] + '</option>');
            }
          }
        }
      });
    ...
    function loadProgrammes() {
      ...
      $.ajax({
        ...
        success: function (data) {
          for (var i = 0; i < data.length; i++) {
            $programmesSelect.append('<option value="' + data[i]['id'] + '">'
                + data[i]['programmeName'] + '</option>');
          }
          $programmesSelect.append(
              '<option value="-1">Définir plus tard</option>');
          loadCountries();
        }
      });
    }
    ...
    function loadCountries() {
      ...
      $.ajax({
        ...
        success: function (data) {
          var currentProgramme = $programmesSelect.find(
              'option:selected').val();
          for (var i = 0; i < data.length; i++) {
            if (currentProgramme === 'Définir plus tard'
                || data[i]['programme']['id'] == currentProgramme) {
              $countriesSelect.append('<option value="' + data[i]['countryCode']
                  + '">' + data[i]['name'] + '</option>');
            }
          }
    ...
    function loadPartners() {
      ...
      $.ajax({
        ...
        success: function (data) {
          for (var i = 0; i < data['data'].length; i++) {
            $partnersSelect.append('<option value="' + data['data'][i]['id']
                + '">' + data['data'][i]['businessName'] + '</option>');
          }
        }
      });
    }
    ...
    function loadDepartureDates() {
      ...
      for (var i = 0; i < 3; i++) { //load 3 years
        $selectDepartureDate.append('<option value="' + 2 + ' ' + (currentYear
            + i) + '">' + Utils.departureStr(2, currentYear + i) + '</option>');
        $selectDepartureDate.append('<option value="' + 1 + ' ' + (currentYear
            + i) + '">' + Utils.departureStr(1, currentYear + i) + '</option>');
      }
    }
  ...
  var RejectMobilityChoiceView = (function () {
    ...
    function loadDenialReasons() {
      $.ajax({
        ...
        success: function (data) {
          for (var i = 0; i < data.length; i++) {
            $selectReason.append('<option value="' + data[i]['id'] + '">'
    ...
    function addOption(e) {
      ...
      for (var i = 0; i < partnerOptions.length; i++) {
        if (partnerOptions[i]['code'] === $optionsSelect.val()) {
          Utils.animate(
              $partnerOptionsDiv.find('div:nth-child(' + (i + 1) + ')'),
              'wobble');
          department.val('');
          return;
        }
  ...
  var MobilitiesView = (function () {
    ...
    function initializeTable() {
      table = $table.DataTable({
        ...
        aoColumns: [
          ...
          {
            mData: function (o, type) {
              return o.nominatedStudent.firstName + ' '
                  + o.nominatedStudent.lastName;
            },
            title: 'Etudiant'
          },
          {
          ...
          },
          {
            ...
            mRender: function (state, type, raw) {
              return '<span class="' + Utils.handleStateClass(state) + '">'
                  + state + '</span>';
            },
            title: 'Etat'
          }],
    ...
    function render() {
      ...
      bindAll();
      $el.find('h2').html(
          ((app.isProfessor()) ? 'Gestions des mobilités' : 'Mes mobilités'));
      NavigationBarView.render();
      if (!table) {
        initializeTable();
      } else {
  ...
  var MobilityDetailsView = (function () {
    ...
    function bindAll() {
      $aClose.on('click', destroy);
      $ulDepartureDocuments.on('change', 'li input[type=checkbox]',
          confirmDocument);
      $ulReturnDocuments.on('change', 'li input[type=checkbox]',
          confirmDocument);
      $ulSoftwareEncodings.on('change', 'li input[type=checkbox]',
    ...
    function unbindAll() {
      $aClose.off('click', destroy);
      $ulDepartureDocuments.off('change', 'li input[type=checkbox]',
          confirmDocument);
      $ulReturnDocuments.off('change', 'li input[type=checkbox]',
          confirmDocument);
    ...
    function updateView() {
      ...
      $el.find('[data-src=type]').html(
          (mobility.type === 'SMS') ? 'Académique' : 'Stage');
      ...
      if (mobility.stateBeforeCancellation) {
        ...
        $el.find('[data-src=stateBeforeCancellation]').html(
            mobility.stateBeforeCancellation);
      }
      if (mobility.professorInCharge.id) {
        $el.find('[data-src=professorInCharge]').parent().show();
        $el.find('[data-src=professorInCharge]')
        .html(mobility.professorInCharge.firstName + ' '
            + mobility.professorInCharge.lastName);
      }
      ...
      for (var i = 0; i < documents.length; i++) {
        ...
        var documentEl =
            '<li><input id="document_' + document.id
            + '" type="checkbox" value="'
            + document.id + '" ' + ((document.filledIn) ? 'checked disabled'
            : '')
            + '><label for="document_' + document.id + '">' + document.name
            + '</label></li>';
      ...
      if (!checkBankDetails()) {
        ...
        if (mobility.firstPaymentRequestDate !== undefined) {
          ...
          if (mobility.secondPaymentRequestDate !== undefined) {
            ...
            $elPayments.find(
                '#second-payment-status .content').html('Effectué le '
          ...
          } else if (mobility.state === STATE_BALANCE_TO_BE_PAID
              && app.isProfessor()) {
    ...
    function updateMobilityState() {
      $el.find('[data-src=mobilityState]').html('<span class="'
          + Utils.handleStateClass(mobility.state) + '">'
          + mobility.state + '</span>');
    }
    ...
    function confirmDocument(e) {
      if (this.checked) {
        ...
        $.ajax({
          ...
          + '/confirmDocument?document=' + documentId + "&version="
          + mobility.version,
          method: 'PUT',
          success: function (resp) {
    ...
    function confirmPayment(e) {
      ...
      $.ajax({
        url: app.API_URL + '/mobilities/' + mobility.id
        + '/confirmPayment?version=' + mobility.version,
        method: 'PUT',
        success: function (data) {
  ...
  var PartnersView = (function () {
        ...
        aoColumns: [
          ...
          },
          {
            ...
            mRender: function (country, type, raw) {
              return '<img src="/images/flags/' + country.countryCode
                  + '.png" class="flags" alt="'
    ...
    function render() {
      ...
      $el.find('h2').html(((app.isProfessor()) ? 'Gestions des partenaires'
          : 'Les partenaires'));
      $el.show();
    }
  ...
  var PartnerDetailsView = (function () {
    ...
    function editDetails(e) {
      ...
      $elDetails.find(
          '.section-content').replaceWith('<form class="section-content">'
          + content + '</form>');
      $elDetails.find('[data-src=legalName]')
      .html('<input type="text" class="input" name="legalName" value="'
          + partner.legalName + '">');
      $elDetails.find('[data-src=businessName]')
      .html('<input type="text" class="input" name="businessName" value="'
          + partner.businessName + '">');
      $elDetails.find('[data-src=fullName]')
      .html('<input type="text" class="input" name="fullName" value="'
          + partner.fullName + '">');
      ...
      if ($elDetails.find('.save.btn.btn-default').length === 0) {
        $elDetails.find('.section-content').append(
    ...
    function editContactDetails(e) {
      ...
      $elEditContactDetails.find(
          '.section-content').replaceWith('<form class="section-content">'
          + content + '</form>');
      $elEditContactDetails.find('[data-src=phoneNumber]')
      .html('<input type="text" class="input" name="phoneNumber" value="'
          + partner.phoneNumber + '">');
      $elEditContactDetails.find('[data-src=email]')
      .html('<input type="text" class="input" name="email" value="'
          + partner.email + '">');
      $elEditContactDetails.find('[data-src=website]')
      .html('<input type="text" class="input" name="website" value="'
          + partner.website + '">');
      if ($elEditContactDetails.find('.save.btn.btn-default').length === 0) {
        $elEditContactDetails.find('.section-content').append(
            '<button class="save btn btn-default">Sauvegarder</button>');
      }
    ...
    function updateView() {
      ...
      $el.find('[data-src="address[postalCode]"]').html(
          partner.address.postalCode);
      $el.find('[data-src="address[country][countryName]"]')
      .html('<img src="../images/flags/'
          + partner.address.country.countryCode
          + '.png" class="flags" alt="Drapeau">'
          + partner.address.country.name);
      ...
      if (app.isProfessor()) {
        if (partner.archived) {
          $buttonArchivePartner.removeClass('btn-danger').addClass(
              'btn-default');
          $buttonArchivePartner.html('Réhabiliter le partenaire');
        } else {
          $buttonArchivePartner.removeClass('btn-default').addClass(
              'btn-danger');
    ...
    function initializeTable() {
      table = $table.DataTable({
        ...
        aoColumns: [
          ...
          {
            mRender: function (data, type, raw) {
              return '<a href="mailto: ' + raw.email + '">' + raw.email
                  + '</a>';
            },
            bSortable: false,
            title: 'E-mail'
          ...
          {
            mRender: function (data, type, raw) {
              return (raw.role === 'Professor') ? ''
                  : '<a href="#" class="promote">Promouvoir</a>';
            },
            bSortable: false,
            title: 'Action'
          }],
      ...
      $.ajax({
        ...
        complete: function () {
          ...
          if (student.birthdate) {
            ...
            $form.find('[name=birthdate]').val(date.getUTCDate() + '/'
